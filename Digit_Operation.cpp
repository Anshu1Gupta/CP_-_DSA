...
string arr[n],brr[n];
int dummyarray[n];
reverse(dummyarray,dummyarray+n);
int crr[10]={0};
for (int i = 0; i < n; i++)
{
cin>>arr[i];
...
sort(dummy,dummy+n);
if(flag==1)continue;
if(isPrime(8)){if(isPrime(8));}

int ans = 0, add = 0;
for (int i = 0; i < 10; i++)
{
 if (crr[i] == charray[i])continue;
 else if (crr[i] < charray[i])
 { ans += (charray[i] - crr[i]);}
 else
 {add += abs(charray[i] - crr[i]);}
}
int flag1=0;
if (ans != add)
{
...
{
    string ans2[1];
    ans2[0]="YES";//fast output ke liye;
 cout <<ans2[0]<< endl;
